...
5. **Isolated Python environments with gVisor** (ROADMAP item 22). The spike is done and its findings and plan are in the roadmap: rootless `runsc run` with a generated OCI config and a skeleton root, not `runsc do`. Next is the implementation (M–L).
