...
func TestAttachCommandsAndMentions(t *testing.T) {
	...
	// queue is empty.
	out = captureStdout(t, func() {
		runTurn(ctx, app, app.Storage.Active().ID, "compare these with @b.png", nil)
	})
	...
	out = captureStdout(t, func() {
		runTurn(ctx, app, app.Storage.Active().ID, "what about @nope.png", nil)
	})
	...
	runTurn(ctx, app, app.Storage.Active().ID, "just text, mail me@example.png", nil)
...
func TestPaste(t *testing.T) {
	...
	runTurn(ctx, app, app.Storage.Active().ID, "what is this?", nil)
	if sentImages(llm) != 1 {
