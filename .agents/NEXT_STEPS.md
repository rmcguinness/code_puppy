...
| `2e4cb2c0`, `a6ed23d6`, `a926faa6` | Phase 7: `internal/server`, approvals over the stream, `code-puppy serve`, the workspace lock |
...
1. **Desktop app: ROADMAP item 23.** Phases 2–4 are done: `internal/app` is a UI-independent core with typed operations, and `tui` only parses and renders. Decided: one service per user, CLI attaches when available, Connect + buf. Phases 5 (`app.Event`) and 5b (per-workspace state) are done. Phase 6 (the protos in `api/codepuppy/v1`, `make proto`, CI checks) is done. Phase 7 (`internal/server`, `code-puppy serve`, the workspace lock, approvals and questions over the stream) is done. Phase 8 (attach: `app.Backend`, `internal/client`, the CLI attaching when the service runs, `--local`) is done. Next: workers (ROADMAP item 24; 24a definitions and 24b enabling and policy are done; 24c running them on schedule in the service are done; 24d1 CLI commands too; 24d2, starting the service at login, is next), then the desktop shell (phase 9). Known remote-only gaps are listed under phase 8b. The phases and decisions are in the ROADMAP item.
...
- **One engine service per user** (2026-09-26, replacing "a process per tab"): `code-puppy serve` hosts every workspace over Connect on a Unix socket; the CLI attaches when it's running, else runs in-process. Scheduled workers (item 24) run in it. See ROADMAP item 23.
