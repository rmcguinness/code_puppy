...
5. **`app.Event`:** `Run`'s handler receives `app` events (text, tool calls and results, approval requests, questions, usage) instead of ADK events, ready to become proto messages; the printer and `--output-format stream-json` consume them.
5b. **Per-workspace state** (required by the per-user service): `config.Load` takes the workspace directory and stops setting `MODENV_PREFIX` in the process environment; `tools.workspace_dir` and `./agents` resolve against the workspace, not the working directory; `-d` stops calling `os.Chdir`; the reply language is per workspace (the interface language stays with the client). Process-wide by design: telemetry, the diagnostic log.
6. **`api/codepuppy/v1` protos**, `buf` configuration, generated Go and TypeScript, CI checks (lint, breaking, generated code current).
