...
7. ✅ **`internal/server` and `code-puppy serve`.** 7a ✅: `internal/server` serves all three services over Connect (`Server.Handler`), opening workspaces lazily through an `Opener` (one `Config` each) and keeping them by symlink-resolved directory; handlers only translate between protos and `internal/app`. Typed errors map to Connect codes with an `ErrorInfo` detail (`toAPI`); errors inside responses (a failed turn, a partial undo) use the same reasons. `RunTurn` streams `accepted`, text and tool events, then `finished`; images are kept per workspace by ID after `LoadImage`/`AddImage`. Tests drive a real HTTP server through the generated clients. 7b ✅: a broker installs the server's approver and question callback on each workspace; `RunTurn` puts the turn's event sender in its context (ADK passes it through to tool calls), so a request becomes an `approval_request` or `question` event with a random ID and waits for `Approve` or `Answer`, or for the turn to end (a client that disconnects ends the turn and denies). A request made outside a turn, where nobody can answer (a future worker), is refused at once. 7c ✅: `code-puppy serve` listens on `~/.code_puppy/run/code-puppy.sock` (`--socket`), created inside a 0700 directory and chmod 0600; it refuses to start when a service already answers there and replaces a stale socket; HTTP/1.1 plus unencrypted HTTP/2 for gRPC clients; SIGINT/SIGTERM stop it gracefully (turns get 10 s), close every workspace and remove the socket. Each workspace it opens gets a freshly loaded `Config`. `app.Open` now takes an exclusive OS lock per workspace (`flock`, `LockFileEx` on Windows; `~/.code_puppy/locks/<hash of the canonical directory>.lock`), released by `Close` or the process's death, so the CLI and the service never own the same workspace; the CLI says so (`ErrWorkspaceBusy`, exit code 2). `server.Client`, `BaseURL`, `Running` and `DefaultSocket` are there for phase 8's attach. Checked by hand as well: `curl --unix-socket` speaks Connect's JSON to it. Originally: Connect handlers over `app.Workspace`, workspaces opened on demand by directory, the Unix socket, a workspace lock, approvals and questions over the turn stream.
8. ✅ **Attach.** 8a ✅: `app.Backend` is the interface front ends drive: every operation they use, plus `SetUI` (approver and question callback), `Processes` (nil when the processes aren't the front end's), `AuditShell` (for `!cmd`), `ImagesEnabled`. `*app.Workspace` implements it; `tui.App.Workspace`, the one-shot runner and the completer take a `Backend`, and no front end reaches internal accessors any more. The interface language's catalogs are `tui.App.Locales` (client state); the one-shot theme comes from the CLI's own config. 8b ✅: `internal/client.Remote` implements `app.Backend` over the service (`Attach(ctx, socket, dir, warn)`, or `AttachHTTP`). Failed calls come back as `app`'s typed errors, rebuilt from the `ErrorInfo` reason (`errors.Is`/`As` work unchanged; sentinels keep the service's message); calls `Backend` gives no error to report go to `warn` and return empty results. `Run` answers the stream's approval requests and questions through the UI set with `SetUI`. Images stay in the service; a `Remote` image carries its metadata and `images.Image.Size`. The protos gained, additively: `GetModelResponse.unavailable` (for `ModelErr`), `GetSettingsResponse.images_enabled`, and `GetSearchProvider`; the server now reports a busy workspace as `WORKSPACE_BUSY`. **Known gaps, remote only:** `Processes` is nil (background processes belong to the service and outlive the client), `AuditShell` does nothing (`!cmd` runs on the client), and `OnFinished` runs when the service's last event arrives, after it collected unread steer messages, so a message sent at that moment waits for the next turn. Tests run the client against a real server, and the REPL's commands and a turn against a remote workspace. 8c ✅: the CLI (REPL and one-shot) attaches when the service answers on its socket (`openBackend` in `cmd/code-puppy/setup.go`), and otherwise opens the workspace in-process as before; `--local` always opens it in-process. An attached REPL says so. The socket is `$CODE_PUPPY_SOCKET` or `~/.code_puppy/run/code-puppy.sock`, for `serve` and the CLI alike. The interface's catalogs are always the CLI's own. Checked with the real binaries too: a piped REPL session attached to `serve` and ran `/model`, `/pin_model`, `/agents`, `/session list` in the service. Originally: a backend interface in `tui`, implemented by `*app.Workspace` and by a client adapter; the CLI attaches when the socket answers. Also moves the last accessor uses (the process list at exit, `!cmd`'s audit entry, the images flag, approval and question wiring in `cmd`) behind operations.
...
2. ✅ **24b, enabling and policy.** `app.Workspace` gains `ListWorkers`, `EnableWorker(name, hash)` and `DisableWorker`, returning `WorkerInfo` (state, schedule as written and as cron, next run when enabled, effective permissions and limits, problems). Workers are found under `[workers] paths` (default `workers`, relative to the workspace). `workers.Store` keeps which workers are enabled at which hash in `~/.code_puppy/workers.json` (atomic writes; one store per process, `app.Options.Workers`); states are new, enabled, disabled, changed (edited since enabling: suspended) and invalid. Enabling requires the hash the caller reviewed to be current (`ErrHashMismatch`). **Trust:** explicitly enabling a worker at a hash is the trust decision; there is no separate per-directory trust setting, and scanning alone never schedules anything. `[workers.policy]`: `allow` (permission kinds; default all five), default and maximum `max_turns` (50/200), `max_cost_usd` (1/10) and timeout (30m/2h), and `max_concurrent` (2); `workers.Apply` drops what the policy doesn't allow and caps the rest, and says what it changed.
3. ✅ **24c, running workers.** 24c1 ✅: `app.Workspace.RunWorker(ctx, name, manual, on)` runs an enabled worker once: in a session of its own (its own `session.Storage`, so the workspace's active session and anyone using it are untouched; `Run` became a wrapper over `run(..., storage)`), with the workflow prefixed by a note that the run is unattended, its effective permissions enforced through `tools.Unattended(ctx, decide)` (which bypasses auto-approval, remembered and saved rules and the interactive approver, and refuses questions), refusals recorded, and its limits applied (`max_turns` through the turn, `timeout` through the context, `max_cost_usd` checked as events arrive). A second run of a running worker is `ErrRunInProgress`. Finished runs go to `~/.code_puppy/worker-runs/<workspace hash>-<worker>.jsonl` (`workers.RunLog`): status (succeeded, failed, limited), duration, cost, model calls, session, refusals, error. **Gaps:** a worker's `agent` and `model` aren't applied yet (the engine runs one active agent per workspace; listings say so); a worker's file changes join the workspace's `/undo` stack; with telemetry on, the workspace's and the run's storage may both write the run session's metadata. 24c2 ✅: `server.WithScheduler(SchedulerConfig{Store, Runs, MaxConcurrent, Rescan})` and `StartScheduler(ctx)` run enabled workers on their schedules with `robfig/cron`, each in its own time zone. Workers are rescanned every minute (and after enabling or disabling) across the workspaces the store lists, so new, edited (now "changed", so unscheduled) and removed `WORKER.md` files are picked up; periodic rescans rather than file watching, for now. A run takes one of `max_concurrent` slots (a scheduled run waits, a manual one fails with `TOO_MANY_RUNS`); a scheduled run while one is going is recorded as skipped; `catch_up: once` runs once on registration when a scheduled time passed since the last run. `WorkerService` is served: `ListWorkers` (one workspace, or every registered one), `EnableWorker`, `DisableWorker`, `RunWorker` (returns as the run starts), `ListWorkerRuns` (running ones first), `GetWorkerRun`, and `WatchWorkerRun`, which replays a running run's events from its start (events aren't kept after a run). `serve` shares one store and run log across its workspaces and starts the scheduler unless `[workers] enabled = false`. `app.RunWorker` takes `RunOptions` (`Manual`, `OnStart`, `OnEvent`).
4. 🚧 **24d, the CLI.** 24d1 ✅: `code-puppy workers` (list), `workers enable <name>` (shows the schedule as understood, the permissions and limits after policy, and the content hash, then asks; `--yes` skips the question; the hash shown is the one enabled, so an edit in between fails), `workers disable`, `workers run <name>` (runs now and prints the run as it goes, then its record), `workers runs <name>`. They go through the service when it's running (`client.Workers`) and otherwise work in-process, as the rest of the CLI does. Worker errors that are the request's fault exit with code 2. Found while testing: `cmd` tests left the developer's API keys visible, so a local test run could call the real provider; `isolate` now hides them. Next, 24d2: a launchd/systemd user unit to start the service at login.
