...
7. 🚧 **`internal/server` and `code-puppy serve`.** 7a ✅: `internal/server` serves all three services over Connect (`Server.Handler`), opening workspaces lazily through an `Opener` (one `Config` each) and keeping them by symlink-resolved directory; handlers only translate between protos and `internal/app`. Typed errors map to Connect codes with an `ErrorInfo` detail (`toAPI`); errors inside responses (a failed turn, a partial undo) use the same reasons. `RunTurn` streams `accepted`, text and tool events, then `finished`; images are kept per workspace by ID after `LoadImage`/`AddImage`. Tests drive a real HTTP server through the generated clients. Still to do: 7b approvals and questions over the stream (`Approve`/`Answer` answer `Unimplemented` until then), 7c `code-puppy serve` (Unix socket, workspace lock). Originally: Connect handlers over `app.Workspace`, workspaces opened on demand by directory, the Unix socket, a workspace lock, approvals and questions over the turn stream.
