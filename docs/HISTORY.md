...
| **Session names** | ✅ autosave names | ✅ named after the first prompt, `/rename`, terminal title, resume command printed on exit | From Antigravity's `/rename`, `/title` and exit hint |
| **Side questions** | ❌ | ✅ `/btw <question>` | Answered in a throwaway copy of the session; read-only; nothing kept (idea from Google's Antigravity CLI) |
| **`/search`** | ❌ | ✅ `/search web <terms>` (top five links read by the agent), `/search session <terms>` (the transcript, including compacted turns) | Search turns are read-only. Session search is literal and doesn't cover tool output; Google search costs aren't in `/cost` (see README, Search) |
...
| **Resilience** | ✅ HTTP retry, MCP circuit breaker / health monitor / retry manager | ✅ retries with backoff for every provider, stall timeouts, MCP restart + circuit breaker + timeouts, parallel tool cap | No MCP health dashboard |
| **Model fallback** | ✅ round-robin model | ✅ ordered `llm.fallback_models`, across providers | Failover (not load balancing): per-model circuit breaker, notices, priced by the answering model |
...
## Appendix: architectural shifts (from the first comparison, 2026-09-22)
